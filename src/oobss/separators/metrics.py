"""
Calculate scale-Invariant signal to distortion/interference ratio.
...
def si_bss_eval(reference_signals, estimated_signals, scaling=True):
    """
    Compute the Scaled Invariant Signal-to-Distortion Ration (SI-SDR) and related measures according to [1].

    .. [1] J. Le Roux, S. Wisdom, H. Erdogan, J. R. Hershey, "SDR - half-baked or well done?", 2018, https://arxiv.org/abs/1811.02508

    Parameters
    ----------
    reference_signals: ndarray (n_channels, n_samples)
        The reference clean signals
    estimated_signal: ndarray (n_channels, n_samples)
...
def _compute_measures(estimated_signal, reference_signals, Rss, j, scaling=True):
    ...
    This implementation was provided by Johnathan Le Roux [here](https://github.com/sigsep/bsseval/issues/3)
